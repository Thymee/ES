function setupButtons(studyViewer) {
	...
	function resetStyle(){
		...
		if($(".blue").hasClass("tool-position")){
			$(".toolbar-button i").removeClass("blue");//移除全部蓝色
			$(".toolbar-button .tool-position").addClass("blue");
		}else{
			$(".toolbar-button i").removeClass("blue");//移除全部蓝色
		}
	};
...
//		resetStyle();
	...
	$(".sub-toolbar").on("click", "li", function(e) {
		...
		//console.log($('#tool-sync').data('sync'))
		e.stopPropagation();
	});
...
//		disableAllTools();
		...
		forEachViewport(function(element){
//			console.log($(element).data('droppable'))
//			console.log($(element).attr('class'))
			if ($(element).data('waiting')||$(element).parents('.imageViewer').data('isLayout')) {
        		return ;
    		}
			
//			var enabledElements = referenceLineSynchronizer.getSourceElements();
//			console.log(enabledElements)
//			referenceLineSynchronizer.remove(element);
//			cornerstoneTools.referenceLiness.tool.disable(element, referenceLineSynchronizer);
			cornerstoneTools.automaticPlanePositioning.tool.disable(element, synchronizer);
...
//			synchronizer.remove(element);
			cornerstoneTools.referenceLines.tools.disable(element, referenceLineSynchronizer);
			referenceLineSynchronizer.remove(element);
//			
//			referenceLineSynchronizer.add(element);
