...
function onImageRendered (e, eventData) {
  ...
  for (let i = 0; i < toolData.data.length; i++) {
    ...
    if (meanStdDev && meanStdDev.mean !== undefined) {
      ...
      let meanText = `平均值: ${numberWithCommas(meanStdDev.mean.toFixed(2))}${moSuffix}`;
      // Create a line of text to display the standard deviation and any units that were specified (i.e. HU)
      let stdDevText = `标准差: ${numberWithCommas(meanStdDev.stdDev.toFixed(2))}${moSuffix}`;
...
function doubleClickCallback (e, eventData) {
  ...
  for (let i = 0; i < toolData.data.length; i++) {
    ...
    if (pointNearTool(element, data, coords) || pointInsideBoundingBox(data.handles.textBox, coords)) {
